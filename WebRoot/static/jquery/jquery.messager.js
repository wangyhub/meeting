...
(function (jQuery){
	...
	this.inits = function(title, text){
		...
		$(document.body).prepend('<div id="message" style="border:#b9c9ef 1px solid;border-radius:8px;z-index:100;width:'+this.layer.width+'px;height:'+this.layer.height+'px;position:absolute; display:none;background:#f5f5f5; bottom:0; right:0; overflow:hidden;"><div style="border:1px solid #fff;border-bottom:none;width:100%;height:25px;font-size:12px;overflow:hidden;color:#1f336b;"><span id="message_close" style="float:right;padding:4px 8px 8px 0;width:30px;line-height:auto;color:red;font-size:22px;font-weight:bold;text-align:center;cursor:pointer;overflow:hidden;">×</span><div style="padding:5px 0 5px 5px;width:100px;line-height:18px;text-align:left;overflow:hidden;">'+title+'</div><div style="clear:both;"></div></div> <div style="padding-bottom:5px;border:1px solid #fff;border-top:none;width:100%;height:auto;font-size:12px;"><div id="message_content" style="margin:0 5px 0 5px;border-radius:8px;border:#b9c9ef 1px solid;padding:10px 0 10px 5px;font-size:12px;width:'+(this.layer.width-17)+'px;height:'+(this.layer.height-53)+'px;color:#1f336b;background:#ffffff;text-align:left;overflow:hidden;">'+text+'<br/><a href="javascript:void(0)" onclick ="openModal()" style="position:fixed;bottom:10px; right:10px; ">查看全部</></div></div></div>');
		
		$("#message_close").click(function(){					setTimeout('this.close()', 1);			var id = $("#dataId").val();			$.ajax({				data:{"id":id},				url:"after/demo/tBzInfo/haveReadData"			});			setTimeout("showTip()",5000);		});		
