...
public class UserUtils {
	...
	public static boolean hasRole(String roleCode){
		...
		if(roleList!=null){
			for(Role role : roleList){
				if(roleCode.equals(role.getEnname())){
					return true;
				}
			}
		}
	...
	public static boolean userHasRole(String roleEnname){
		User user = getUser();
		List<Role> roleList= user.getRoleList();		
		if(roleList!=null){
			for(Role role1 : roleList){
				if(roleEnname.equals(role1.getEnname())){
					return true;
				}
			}
		}
	 ...
	 * @return
	 */
	public static List<Menu> getMenuListByRole(String role){
		...
		m.setUserId(user.getId());
		m.setName(role);
		menuList = menuDao.findByUserRole(m);
